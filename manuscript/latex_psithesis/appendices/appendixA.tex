% Converted from: manuscript/draft_v5_en/back/A_appendix_hazards.md
% Appendix A --- Hazard register (extended version)

\chapter{Hazard Register (Extended Version)}
\label{app:A}

Complete version of the register summarised in \Cref{tab:4.1}. Each entry keeps its
...
\begin{registertable}{@{}L{9mm}L{40mm}L{13mm}L{13mm}L{8mm}L{15mm}L{11mm}L{27mm}L{27mm}@{}}
  \caption{Hazard register, extended.}\label{tab:A.1}\\
  \toprule
  \tabhead{ID} & \tabhead{Hazard (description)} & \tabhead{S} & \tabhead{E} & \tabhead{C} &
  \tabhead{Criticality} & \tabhead{Applicable ODDs} & \tabhead{Main operational consequence} &
  \tabhead{Dominant root cause hypothesis}\\
  \midrule
  \endfirsthead
  \toprule
  \tabhead{ID} & \tabhead{Hazard (description)} & \tabhead{S} & \tabhead{E} & \tabhead{C} &
