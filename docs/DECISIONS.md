...
**Context.** The platform team delivered the first bench characterisation of the physical
CobraFlex 1:14 (*CobraFlex 1:14 Parameters_0813*): mass, CoG, an inertia tensor, chassis
geometry, dynamic maxima, and two validation campaigns — straight-line distance and in-place
rotation, both over 10 s. Until now `docs/14` §2.2 read literally *"Total mass to be measured
from real car. Inertias to be measured from real car."*, and `docs/17` §2 item 4 carried
`steering_to_yaw_rate_gain` as an open `[VERIFY]`. This is the first hardware evidence to reach
the specification.

**What the data actually says.** Three of the supplied rows are measurements, three are not,
and the distinction is the substance of this decision.

*Measured and consistent:* total mass **3.5 kg**; wheelbase **0.154 m**, track **0.153 m**,
wheel radius **0.03725 m**; straight-line tracking **≈0.99** of commanded up to 0.53 m/s
(1.998/2.000, 3.964/4.000, 5.207/5.300).

*Measured and contradicting the simulator:* in-place rotation delivers only
**0.4954 × commanded yaw** — least squares through the origin over four commands
(0.20/0.40/0.53/0.80 rad/s), per-point gains 0.485/0.500/0.495/0.495, linear with no offset.
Gazebo's DiffDrive plugin delivers ~1:1. The forward axis transfers at 0.99 and the rotational
axis at 0.50, so this is scrub, not a speed error: four fixed wheels dragging sideways, giving
an effective track of `0.153/0.4954 = 0.309 m`, ≈2.02× the physical track.

*Not measurements, recorded so they are not mistaken for evidence:*

- The **inertia tensor** `0.008542 / 0.023160 / 0.028702` reproduces `(1/12)·m·(…)` for **this
  repo's own URDF chassis box at the old 5.0 kg** to nine decimals. It is our assumption read
  back out of Isaac after a URDF import — circular, and internally inconsistent with the 3.5 kg
  measured beside it.
- **`v_max 0.53 m/s` and `ω_max 6.0 rad/s`** are the serial driver's clamp constants already
  recorded in `docs/17` §2 item 3. The 6.0 figure is not reachable: ideal diff-drive gives
  `2·v_max/T = 6.93 rad/s`, and ≈3.4 rad/s after the measured scrub. The campaign only reached
  0.396 rad/s achieved.
- The **CoG** `(0.006, −0.004, 0.030) m` is frame-ambiguous and, read in `base_link`,
  unreachable by this link layout: with the 0.19 kg lidar at z = 0.16 m, placing *all* remaining
  mass at z = 0.030 m still floors the composite at 0.037 m.

**Decision — four parts.**

**(1) Correct the URDF mass budget to 3.5 kg, distributed from the bill of materials.** It
summed to **6.59 kg** — chassis 5.0 + body 1.0 + 4×0.1 wheels + 0.19 lidar — i.e. **1.88× too
heavy** through F4, GE4-V2 and `campaign_2d_ppo550k`.

The first cut was a **uniform** scale `k = 3.5/6.59`, on the reasoning that only the total was
measured. Later the same day the platform team supplied the **itemised bill of materials** — the
three printed body shells weighed individually (91.3 g bottom / 118.5 g centre / 68.0 g top
cover) plus the three bought-in components — and that supersedes it, because it shows the
**entire 3.09 kg overshoot sat in one link**: the `chassis_mass` placeholder. `body_link`,
`lidar_link` and the wheels were already right, so the uniform scale had taken three correct
values and made them wrong (most visibly the lidar, driven to 0.101 kg when the manufacturer
figure is 0.190 kg and the original URDF already had it exactly). The uniform scale was replaced
with the itemised one:

| Link | Value | Composition | Confidence |
| --- | --- | --- | --- |
| `body_link` | 0.8928 | PLA 0.2778 (measured) + powerbank 0.550 + ZED Mini 0.060 + lane cam ≈0.005 | measured + datasheet |
| `lidar_link` | 0.190 | RPLIDAR A2M4, manufacturer — restored to the original value | manufacturer |
| wheel ×4 | 0.1 | **not measured**; left at the original value | assumption |
| `base_link` | 2.0172 | remainder: frame + 4 motors + driver board + motor battery + Jetson DevKit 0.175 + wiring | derived total; Jetson placement **confirmed** on the chassis (platform team, 17.08.2026) |

Two figures quoted with the parts were corrected against datasheets already in the project's
own reference folder: the **powerbank is 550 g, not 500 g** (XTPower XT-27000DC, *"Weigth 550g"*)
and the **Jetson Orin Nano DevKit is 175 g** (NVIDIA SP-11324-001 v1.3 p33), previously
unaccounted for anywhere. The ZED Mini's 60 g is folded into `body_link` because
`zed_macro.urdf.xacro` declares **no `<inertial>` at all** — the ZED is a massless link in every
URDF, and correcting that properly means editing a vendor description. Inertias in the two
generated Isaac files were recomputed from the same primitives the generator uses, verified
beforehand to reproduce the previous values exactly at the previous masses.

**2.25 kg — 64 % of the vehicle — remains a single derived remainder.** The rolling chassis has
never been weighed. The two measurements that would close the budget are **one wheel** and **the
bare chassis**; requested from the platform team.

**(2) Do not apply the CoG or the inertia tensor.** Both fail the check above. On the CoG the
itemisation is decisive rather than merely suggestive: the powerbank (550 g, heaviest single
item) is in the *centre* shell and the lidar (190 g) on the *top* cover, so **740 g — 21 % of the
car — is demonstrably in the upper two layers**. The itemised composite sits at **0.0566 m**
above `base_link` (0.0938 m above ground), nearly 2× the claimed 0.030 m. Read from the **chassis
box centre** the supplied figure would land at 0.060 m, **3.4 mm from the model** — close enough
to be a confirmation of the reference frame rather than a coincidence, but it stays a hypothesis
until the platform team confirms. A real inertia tensor still needs a bifilar-pendulum or CAD
measurement.

**(3) Compensate the yaw gap at the command side on hardware, and nowhere else.**
`deploy_cobraflex.launch.py` raises `steering_to_yaw_rate_gain` from 0.8 to
**`0.8 / 0.4954 = 1.615`**, so the *achieved* yaw on the car matches the 0.8 rad/s the trunk
policy and the cage were trained and verified against. The `vehicle_control_node` default stays
**0.8** — that is the sim value and every frozen verdict was produced against it. Two caveats are
carried in the launch file and in `docs/17`: scrub is **surface-dependent** (0.4954 came from a
bench surface, not the lane circuit), and the new default turns the car ~2× harder, so bring-up
runs at 0.22 m/s with the e-stop in hand.

**(4) Leave `wheel_separation = 0.154` alone, but record why.** It is the measured *wheelbase*;
the diff-drive kinematics want the *track*, 0.153 m. The two agree to 0.65 % on this chassis,
which is why the mix-up never surfaced. Correcting it would perturb the plant that produced every
frozen verdict for a 0.65 % gain, three orders of magnitude below the yaw error on the same axis.
Provenance comments added in `robot.gazebo`, `tools/isaac_scene.py`, `docs/13` and `docs/14`.

**Consequence — this is transfer risk T2, now quantified.** Every cage margin expressed through
steering buys **half** the physical yaw it buys in Gazebo. At `steer = 1.0`: 0.800 rad/s sim →
0.396 rad/s real. Minimum turn radius at 0.22 m/s: 0.275 m → 0.555 m. C-06's
`delta_max_steering_per_cycle = 0.15` bounds yaw acceleration at 2.40 rad/s² sim → 1.19 rad/s²
real (20 Hz). The tightest `complex_b` curve (driven `R_min ≈ 0.998 m`) needs 0.220 rad/s at
0.22 m/s: **27.6 % of full steer in sim, 55.6 % on hardware**. Still feasible — the headroom drops
from 3.6× to 1.8×, it does not vanish.

**Consequence — the two simulators now bracket the truth, and Isaac gets a calibration target.**
Gazebo ≈1.00, real **0.4954**, Isaac ≈0.18 at `friction 0.05` (D-54). Gazebo is ~2× optimistic,
Isaac ~2.75× pessimistic. D-54 raised Isaac's `cage.yaw_gain` to 2.4 to paper over a plant that
was too slippery; the physically-correct fix is to tune friction until the `--turn` test returns
**≈1.44 rad/s** (0.4954 × the 2.9 rad/s ideal) and then restore `yaw_gain` to 0.8. Recorded in
`docs/13` with the caveat that one isotropic friction knob probably cannot match both axes — the
real car is 0.99 forward / 0.495 yaw, while Isaac at 0.05 is already down to 65 % forward at only
0.18 yaw, so **anisotropic wheel friction** is the likely requirement. Not yet attempted.

**Explicitly NOT closed.** **TBD-Q10 (`ODD-3.A_LAT_MAX`)** — no lateral-accel envelope was
supplied, so **M-4 remains open** and `docs/08` stays below v1.0 (D-33 unchanged). The camera
`[VERIFY]` items remain the highest-priority open risk: the document contains **nothing about the
camera**, and `docs/17` §2 item 1 — the 90° effective HFOV, which scales every `ey` in metres the
cage acts on — is still the load-bearing unverified number of the whole transfer. Also still open:
...
**Verification.** `python tools/check_traceability.py` → All checks PASSED. Mass totals asserted
programmatically at 3.5000 kg in `cobraflex_isaac.urdf` and `physics.usda`. `pytest` unaffected
(no cage or policy logic touched). **Not verified:** neither URDF has been loaded in Gazebo or
Isaac since the rescale — `xacro` is unavailable on the Windows authoring host, so
`tools/build_isaac_urdf.py` could not be re-run, and `cobraflex_isaac.urdf` + `physics.usda` are
hand-patched generated artefacts. Regenerate both on the Ubuntu host before any Isaac work.
...
* **Amended by D-79 (31.08.2026).** The order of work stands; the *target* moves. The capture session
  measured the failure as a property of **location**, with the defect in **candidate generation**
  rather than pair selection — so the widening must act on line extraction, and a temporal continuity
  prior over the selected pair is rejected as the first fix.
...
### D-79 — The capture session lands: the D-43 estimator's failure is a property of WHERE the car is, not of what it is doing, and the defect is candidate generation rather than pair selection

| Field | Value |
| --- | --- |
| Section | `experiments/physical/datasets/{truepos_*,parked_seg*}/`; `experiments/physical/datasets/CAPTURE_NOTE_20260831.md` (new); `docs/17` §12 |
| Status | ACCEPTED — findings. **D-78's acceptance criterion FAILS**, so D-75 stays blocked and C-04 stays un-armable. No `cage.yaml`, code, SR, cage rule, ODD parameter or verdict touched; `verdict_phys` stays open. Posterior evidence. |
| Date | 31.08.2026 |

**Context.** D-78 built the capture and fixed its acceptance band **in advance**: `∫|κ|ds / (laps·2π)`
in 0.75–1.35, scored per station segment with the worst segment named and no circuit mean. It was
executed the same afternoon, camera only, rectified, with arc length from four tape-measured floor
stations and true offset held by a chassis pointer against the painted lines (no chalk was available —
one transfer error *fewer*, since a chalk line would have been measured off the same paint). Four
measurement laps and four parked probes, ~11 500 frames.

**Finding 1 — the criterion fails, and the failure now owes nothing to odometry.** Four laps score
**1.97, 2.01, 2.25 and 2.37**; restricting the integral to right-pair frames gives **1.78–2.22**. D-75
estimated ~3× from driving logs; with the odometry, the policy, the cage and (in the `ctrl` laps) the
operator all out of the chain the figure is **~2×**. Same defect, smaller, and measured against a
tape. **C-04 stays un-armable.** D-76's offset suspicion is confirmed in the one direction tested:
0 → −65 mm takes paired frames 68.2 → 46.6 % and the ratio 2.01 → 2.25.

**Finding 2 — the operator's shadow was a real confound worth a third of the effect.** The first lap
was pushed with the operator walking alongside at 0.13 m/s; the two `ctrl` laps from outside the lane.
Paired 55.4 → 69.1 %, `single_line` 44.1 → 30.1 %, `|ey|` error 54.6 → 31.9 mm, its p95 213 → 91 mm.
The capture protocol becomes *push from outside the lane*, and the 18.08 `circuit_export` dataset
(95.3 % paired, also hand-swept) is no longer above suspicion — not refuted, not comparable directly
(unrectified, different hour), simply no longer clean.

**Finding 3 — it is the place, not the motion. This is the decision's core.** Each lap's fifth segment
sits at the start of the straight — where the 31.08 sweep and every `lanecheck` were taken — and it is
excellent (92.7 % paired / 15.9 mm at `ey` = 0; 96.7 % / **7.2 mm** at −65 mm) while the moving
segments are not (37.9–60.2 % paired, 49.7–63.4 mm). Motion and location were confounded, so the car
was **parked inside the bad stretches** at true 0 and held still 20 s: `parked_seg23_b` **0.0 %
paired** (blind), `parked_seg23_a` 17.3 % with a 711 mm span, `parked_seg34_a` 99.4 % paired but
**+43.7 mm** wrong, `parked_seg34_b` 100 % paired and **−39.7 mm wrong with sd 3.1 mm**. Parked in the
bad places is as bad as driving through them, or worse.

Consequence, stated plainly: **the location every earlier single-pose conclusion was drawn from is the
estimator's best point by a wide margin.** Their *content* stands; their *generalisation to the
circuit* is retracted. `SWEEP_NOTE.md` declared exactly this limitation about itself, and it is now
measured rather than suspected. D-71 §3's method lesson — match the measurement to the quantity —
claims a fifth scalp.

**Finding 4 — the mechanism is candidate GENERATION, and it inverts the obvious fix.** `line_c0_m`
(the column D-78 added) makes every detected line's intercept readable, and for a car at true 0 the
right pair is ≈ ±125 mm. `parked_seg23_b` sees **exactly one line** in 273/273 frames — a *detection*
failure, nothing to select from. `parked_seg23_a` sees three at ≈ −200 / −150 / +250 mm: the first two
are 50 mm apart, i.e. **the two edges of one painted stripe**, and no pair has a plausible separation,
so the estimator correctly refuses (`single_line`, 306/370). `parked_seg34_a` pairs ≈ (−200, +100) →
width 300.6 mm, midpoint +50; `parked_seg34_b` pairs ≈ (−100, +200) → width 279.2 mm, midpoint −50.
At these places the candidate set is not the two stripe centres but stripe *edges*, adjacent markings,
or one stripe; the surviving pair carries a plausible 279–301 mm width with a midpoint displaced
40–50 mm.

**Decision.** D-76's order of work — widen the estimator before narrowing the policy — **stands, and
its target moves**: the widening must act on **line extraction** (stripe edge versus stripe centre,
threshold behaviour at the failing locations), not on pair selection. A temporal continuity prior over
the selected pair, the first thing D-76 would naturally have reached for, is **rejected as the first
fix**: `parked_seg34_b` proves the wrong pair is stable to **3.1 mm**, so a continuity prior would
track it happily and convert a detectable flip into an undetectable bias.

**Consequences.**

* **No dispersion statistic can gate this.** `parked_seg34_b` is 39.7 mm wrong with sd 3.1 mm and span
  13.3 mm: the old `sd_ey ≤ 10 mm` gate passes it comfortably, the 31.08 span gate (`≤ 12 mm`) fails it
  by 1.3 mm — a coin flip. Only `lanecheck --true-ey` catches a bias, and it must be run **at more than
  one location**; "lanecheck at −60 and −100 mm" is necessary but not sufficient, because at the start
  of the straight it will keep passing.
* **`kappa_ahead` over-reads with the car stationary** (`parked_seg34_a`, mean |κ| 0.88 m⁻¹). D-76's
  instruction to re-derive every physical analysis binned on `κ` is unchanged, and the reason is now
  geometric rather than statistical: the fitted lane is built from the wrong candidate lines.
* **The per-segment map does not reproduce and must not be cited as one.** Across three offset-0 laps
  only the 2→3 stretch repeats as poor (paired 50.2 / 51.4 / 46.7 %) and the start straight as good;
  3→4 swings 75.4 / 98.6 / 70.7 %. An early reading of the first lap called 3→4 a located defect —
  three laps do not support it.
* **Still owed:** the `+60`, `−100` and `+100` laps of D-78's matrix, and an inspection of what is
  physically different at the four probed spots (stripe width, a mat join, the adjacent red carriageway
  entering frame, local lighting). The latter is cheap and is the highest-value next measurement.
* No hazard, SR, cage rule, scenario, metric or verdict is added or re-valued. The policy never ran.

Cites D-43 (the estimator under test and its H-12 residual), D-71 (§3's method lesson), D-75 (the
closed-loop precondition, still unmet), D-76 (the work order this amends), D-77 (the relocation
statistic reproduced here at 4.5–5.1 % on shadow-free laps), D-78 (the protocol and the acceptance
band it fixed in advance), D-69 (the verdict of record, untouched).

---

### D-80 — The bare-policy arm settles what Phase 5 has been arguing: the binding constraint is the measurement, not the control. D-74's reset parameters do not survive contact with motion

| Field | Value |
| --- | --- |
| Section | `src/cobraflex_rl/launch/deploy_cobraflex.launch.py` (`control_emergency_topic`, new; `value_type` fix); `experiments/physical/runs/lap_{mon,mon_escape,mon_hold03,bare}_20260831*/`; `docs/17` §13 |
...
right; more mildly entering the middle of the first curve). So the three blockers above are not a
...
bare-policy arm required either a code edit or killing a node by hand. The second leaves **no trace in
the run's evidence**, which a traceability-first repo cannot accept. The argument defaults to
`/emergency`; pointed at an unpublished topic it detaches that one link while the cage keeps
evaluating, publishing and logging every rule. It carries its own warning: *DIAGNOSTIC ONLY — with it
set, nothing in software stops the car.*

**Consequences.**

* **This arm can never be `verdict_phys`** — no safety function is active while it runs. N = 1,
  `near_secant` rather than the D-43 contract (§8.4), and the operator repositioned the car by hand
  after each departure, so the run's `ey` statistics and any automated departure count are
  contaminated **by design**. The steering population and the latched-versus-commanding ratio are not.
* **Two launch defects found by launching, not by testing.** `value_type=list` in D-74's
  `blocking_rules` guard (fixed to `List[str]`) and a missing `setup_deploy_env.sh`. Both are the
  repo's own rule — *pytest is not evidence a node runs* — collecting for the third time today.
* **D-74 is amended in its parameters, not in its architecture:** the reset path stays outside the
  cage, C-05 stays unchanged, and `observe` stays the default. What this measures is that the 1 s hold
...
these three runs exercise), D-76 (the ordering this supplies a positive control for), D-79 (the
locations these failures land in).
