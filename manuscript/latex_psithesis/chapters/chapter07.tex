...
\figwide{fig_7_2_intervention_newcam_notitle}{Cage activity over the one-dimensional training. Above, the total intervention rate falls from $\sim$87\,\% to $\sim$40\,\% while the emergency rate is nil from the first moment. Below, the per-rule breakdown shows what that rate is made of: it is the rate limiter, and the safety rules \cagerule{01}, \cagerule{02}, \cagerule{03} and \cagerule{05} fall to zero in the first steps and stay there.}{7.2}

A second result of this training, which \Cref{fig:7.2} breaks down, is the co-adaptation between policy and cage: the intervention rate falls from $\sim$87\,\% at the start to $\sim$40\,\%, dominated by the rate limiter, while the safety rules fall to zero. The reading is that the policy learns to respect the safety constraints --- it does not approach the edge --- but its steering command is still jerky and the limiter smooths it continuously.

The deterministic nominal evaluation against a classical controller on the same circuit, collected in \Cref{tab:7.1}, gives the result that justifies the cost of the learned component:
...
\paragraph{Algorithm} A policy trained with the on-policy method reaches a mean reward of 1755 at around 472,000 steps (\Cref{fig:7.4}) with a high and stable plateau, while the off-policy method never exceeds $\sim$200 and does not manage to master the circuit.

\paragraph{Speed ceiling} A single-variable comparison shows that at 0.5 m/s the policy peaks at 654 and drives dirty --- it overshoots the tight curves --- against 1421 at 0.22 m/s, where it takes them cleanly.\sidenote{An honesty warning about the figures: the reward is not comparable one to one between action spaces, because the episode ceiling doubles when moving to two dimensions. The factor of $\sim$2 with respect to the one-dimensional policy is mostly greater survival and horizon, not \enquote{twice as good driving}.}

\figwide{fig_7_4_ppo2d_training_curve}{Training reward of the two-dimensional reference policy against the one-dimensional one and against the off-policy variant. Peak 1755 and a high stable plateau, against the post-peak collapse of the first one and the ceiling of $\sim$200 of the second one. The candidate checkpoints evaluated are marked.}{7.4}

\subsection{Checkpoint selection: by driving, not by reward}
\label{sec:7.5.2}

During the whole training the cage stays latent in safety: no activation of the lateral limit, heading, predictive or emergency rules; only of the rate limiter. The selection was resolved by evaluating three candidates in closed loop, and the result confirms the lesson of \Cref{sec:7.4} as clearly as possible: the checkpoint at the reward peak is the worst of the three --- fourteen safety interventions and 49 mm maximum lateral error --- while the one at 550,000 steps wins clearly: 5.32 laps, 8.6 mm mean error, 27 mm maximum, zero emergencies and zero safety interventions.
...
\figwide{fig_7_5_ppo2d_action_distribution}{Distribution of the raw action at the start against the end of training, one panel per dimension. In steering, the initial all-or-nothing command dissolves (36.9\,\% $\rightarrow$ 7.1\,\% of saturated samples). In throttle, the evolution is the opposite, towards saturation (48.2\,\% $\rightarrow$ 89.6\,\%): the policy learns to ask for the ceiling almost always.}{7.5}

\Cref{fig:7.5} supports the honest reading of the longitudinal authority: the policy uses it to set the speed regime, not to draw a profile. There is modulation and it is well localised --- the 8.3\,\% of steps with reduced throttle are concentrated at high curvature and rise to 35.6\,\% at the tightest apex --- but its magnitude is marginal: the throttle drops to 0.81 and the speed falls only from 0.218 to 0.216 m/s. The policy arrives at the tightest curves practically at the ceiling. This apparently minor observation explains one result of \Cref{ch:08}: the speed rule of the cage never activates during the whole campaign.
