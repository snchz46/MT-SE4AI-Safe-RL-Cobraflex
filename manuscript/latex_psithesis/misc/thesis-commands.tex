...
% Part dividers
% ---------------------------------------------------------------------------
% The Markdown groups the twelve chapters into four parts (Part I--IV); md2tex.py
% writes each divider as \thesispart{Title}{what it groups}. The book class's
% \part sets the page and the number; the second argument goes under the title
% and stays out of the table of contents. No \par inside the title: nameref
% reads it with a non-\long macro. The \parbox keeps the subtitle's line
% spacing its own instead of the \Huge of the title around it.

\newcommand{\thesispart}[2]{%
  \part[#1]{#1\\[3ex]\parbox{\textwidth}{\centering\normalfont\large #2}}}

% The ToC line for a part, in the idiom of the class's chapter line
% (PSIThesis.cls): the number in small caps hanging in the left margin, and
% \nobreak so the line never ends a ToC page apart from its first chapter.
\etocsetstyle{part}
   {}
   {}
   {\addvspace{2ex}%
    \makebox[0pt][r]{\textsc{\MakeTextLowercase\partname\ \MakeTextLowercase\etocthenumber}\hspace{\tocsep}}%
    {\large\bfseries\color{ubblue}\etocname}\par\nobreak\vspace{3ex}}%
   {}
...
\newcommand{\prelimflag}{%
  ...
  One run, \mon{} mode, not scored under the scenario protocol.}}
...
% Float placement
% ---------------------------------------------------------------------------
% The LaTeX defaults (\topfraction 0.7, \textfraction 0.2, \floatpagefraction
% 0.5) are tuned for small floats. This thesis has many tables and figures
% that span text + margin and take a third to two thirds of a page; with the
% defaults they drift onto float-only pages and leave half-empty text pages
% behind. These values let a large float share a page with text.
...
% A little extra stretch before TeX gives up on a line: the text column is
% only 107 mm and the prose is dense with unbreakable identifiers and citation
% labels, so without it a few justified lines stick out into the margin.
