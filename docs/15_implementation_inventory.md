...
| `src/cobraflex_rl/config/train_sac_camera.yaml` | E-track SAC training (D-60): mirror of `train_ppo_camera.yaml` differing only in `algorithm: sac` + the `sac:` off-policy block (`buffer_size` stays explicit — the camera replay buffer holds ~56 KB/transition) |
| `train_{ppo,sac}_camera_pilot25k.yaml` | 25k PPO-vs-SAC verification pilot pair (seed 2024, 15.07.2026); identical except the algorithm switch — evidence under `experiments/sim/training/pilot25k_ppo_vs_sac_2024/` |
