...
| [train_ppo.py](../src/cobraflex_rl/cobraflex_rl/train_ppo.py) | Training entry point: config/centerline load, env build + `check_env`, `VecFrameStack`/`VecNormalize` wrapping, PPO construction (all hyperparameters from YAML, `target_kl` / `lr_schedule` / `clip_range_vf` stability levers), `--resume-from`, reproducibility `metadata.json` + checkpoint-registry append |
| [callbacks.py](../src/cobraflex_rl/cobraflex_rl/callbacks.py) | SB3 callbacks: progress bar, `LearningCurveCallback` (reward + PPO health + per-rule cage-intervention rates per rollout → `learning_curve.csv`), `ActionSampleCallback` (raw-steering histogram evidence) |
| [training_metrics.py](../src/cobraflex_rl/cobraflex_rl/training_metrics.py) | Pure schema/aggregation behind the learning-curve CSV (testable without SB3) |
| [eval_policy.py](../src/cobraflex_rl/cobraflex_rl/eval_policy.py) | Scored deterministic evaluation of a checkpoint through the *same* env + cage; emits §7.5 evidence under `experiments/sim/runs/<run_id>/` |
