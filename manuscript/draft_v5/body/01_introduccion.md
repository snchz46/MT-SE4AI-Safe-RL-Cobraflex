...
La conducción autónoma ha pasado en una década de demostración de laboratorio a producto comercial parcial. La mayoría de los fabricantes ofrece ya sistemas avanzados de asistencia al conductor (ADAS) de nivel 2 SAE y avanza hacia los niveles 3 y 4 (Cheng et al., 2025), y existen ya los primeros ejemplos de tecnología de nivel 4 (Paniego et al., 2024). La Figura 1.1 reproduce la escala de referencia con la que se nombran esos grados de automatización a lo largo de este trabajo. En ese movimiento se han consolidado en paralelo dos tendencias: la incorporación creciente de componentes basados en aprendizaje automático —redes profundas, revisadas para el control del vehículo por Kuutti et al. (2021a), y, más recientemente, políticas entrenadas por refuerzo (Kendall et al., 2019)— en módulos críticos de percepción, decisión y control; y el endurecimiento de los marcos normativos de seguridad funcional.
...
*Figura 1.1 — Los seis niveles SAE de automatización de la conducción. El caso de estudio de esta tesis se sitúa en el espacio de las funciones de nivel 2 (§1.6.1). Reproducida de la tabla resumen de SAE J3016™, © 2021 SAE International, que puede copiarse y distribuirse tal cual siempre que se acredite a SAE International como fuente.*

Ambas tendencias son individualmente sólidas y presentan una tensión estructural cuando confluyen. Los marcos clásicos —ISO 26262:2018 a la cabeza— fueron diseñados para sistemas cuyo comportamiento es derivable de una especificación escrita a priori, verificable mediante tests con salidas esperadas y validable estáticamente antes del despliegue. Los componentes aprendidos rompen los tres supuestos: su comportamiento emerge de una optimización estocástica, su salida no admite la noción clásica de «respuesta correcta» y su robustez fuera de la distribución de entrenamiento solo puede caracterizarse empíricamente (Wäschle et al., 2022; Paterson et al., 2025). Uno de los primeros análisis sistemáticos de esta tensión es el de Salay, Queiroz y Czarnecki (2017), que identificó cinco áreas concretas de impacto sobre ISO 26262 —desde la aparición de nuevos tipos de hazard hasta la constatación de que cerca del 40 % de las técnicas de software de la Parte 6 aplicables a nivel de unidad no aplica en absoluto a componentes ML— y constituye el antecedente conceptual directo de este trabajo. La investigación y la industria han respondido con dos estrategias parciales: contención mediante arquitecturas monitor-actuador o *safety cages* (Kuutti et al., 2019, 2021b) y caracterización mediante validación basada en escenarios (De Gelder et al., 2024). Su integración coherente dentro de un ciclo de desarrollo trazable sigue siendo un problema abierto.
...
Se reclaman cinco aportaciones, etiquetadas C1–C5. No son lo mismo que las cinco *adaptaciones* A1–A5 que componen el marco y que se definen en §3.4: las adaptaciones son el contenido del artefacto, las aportaciones son lo que este trabajo reclama añadir sobre el estado del arte. A1–A5 designan siempre adaptaciones, en este capítulo y en todos los siguientes.

- **C1 — Marco metodológico unificado.** Un V-Model adaptado con cinco modificaciones explícitas (A1–A5): desdoblamiento del diseño de módulo en *Cage Specification* y *Training Specification*; desdoblamiento del test unitario en *Cage Unit Tests* y *Policy Behavioral Evaluation*; introducción de un nivel de monitorización en operación como validación continua; trazabilidad bidireccional como restricción dura; y reformulación de la validación operacional con caracterización explícita del gap sim-to-real.
- **C2 — Operacionalización ejecutable.** Cada modificación se acompaña de los artefactos que la materializan, con plantillas reutilizables y validadores automáticos —en particular el verificador de trazabilidad que la convierte en una restricción mecánica de puerta.
- **C3 — Caso de estudio completo y reproducible.** Aplicación del marco a un sistema implementado desde cero, en dos instanciaciones cuyo contraste aísla el coste de la percepción por cámara, con artefactos versionados, scripts de entrenamiento y evaluación y datos de ejecución publicados.
- **C4 — Caracterización empírica del gap sim-to-real** en peldaños de fidelidad creciente: Gazebo (campaña de referencia) → un simulador de mayor fidelidad (Isaac Sim, PhysX + RTX) → plataforma física. El tercer peldaño cierra como puesta en marcha y no como campaña de resultados, y los Capítulos 9 y 10 declaran ese límite en lugar de absorberlo.
- **C5 — Auto-evaluación del marco:** coste de adopción, puntos donde funcionó como se esperaba y puntos donde reveló limitaciones, como evidencia para refinamientos posteriores por terceros.
...
- **El peldaño físico cierra como puesta en marcha.** La cadena de despliegue funciona sobre el vehículo real y produce resultados de calibración y hallazgos estructurales, pero ningún escenario se ha puntuado sobre hardware y la cage nunca ha modificado una acción allí. La columna física de la tabla de veredictos se declara por ello *no ejecutada* (§10.4) y toda cifra de conducción del Capítulo 9 es preliminar. Es un límite de la evidencia, no del marco, y así se declara allí donde aplica.
...
La tesis se organiza en doce capítulos agrupados en cuatro bloques, resumidos en la Figura 1.2. El Bloque I — Marco comprende esta introducción, el estado del arte (Capítulo 2) y la metodología (Capítulo 3), que constituye la aportación académica central. El Bloque II — Especificación cubre el dominio operacional, el análisis de peligros y la derivación de requisitos (Capítulo 4) y el diseño arquitectónico con la especificación de la cage (Capítulo 5). El Bloque III — Implementación y evaluación recorre la implementación y verificación (Capítulo 6), la especificación de entrenamiento y su ejecución (Capítulo 7), la campaña de evaluación experimental (Capítulo 8) y la caracterización del gap sim-to-real en peldaños de fidelidad creciente (Capítulo 9). El Bloque IV — Cierre presenta la validación operacional y la tabla consolidada de veredictos (Capítulo 10), la discusión del propio marco frente a sus criterios (Capítulo 11) y las conclusiones y trabajo futuro (Capítulo 12).

<img src="../figures/fig_1_2_document_roadmap_es.png" alt="Figura 1.2 — Mapa de lectura del documento." width="620"/>

*Figura 1.2 — Mapa de lectura del documento: los cuatro bloques, los doce capítulos, lo que deja cada uno y el nivel del V-Model adaptado que ocupa. La correspondencia formal entre niveles y artefactos no se repite aquí — se da en la Tabla 3.3, una vez definido el marco.*

Los anexos recogen el material de evidencia que sustenta el cuerpo sin interrumpirlo: el registro de peligros extendido (A), la especificación de requisitos con su *rationale* (B), las elecciones de instrumento y el mapeo normativo (C), la especificación del dominio operacional (D), los parámetros de la cage (E), la matriz de trazabilidad (F), el espacio de posicionamiento completo (G), el detalle de la especificación de entrenamiento (H) y el desglose escenario por escenario de la campaña de referencia (I).
