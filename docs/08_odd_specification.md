...
**Version:** 0.9.4 (Phase 5 closed, 01.09.2026 — no ODD parameter added or re-valued; the
0.4954 yaw gain is superseded *in shape* by M-7 §5's compressive measurement, and **TBD-Q10 is
closed as open** because M-4 was never taken and is not planned, so this document stays below
v1.0 **by design**, per D-33. Previous: 0.9.1 = pre-deployment 2-D verdict of record,
31.07.2026 — the qualification
...
| **0.9.4** | **2026-09-01** | **SS** | **Phase 5 closed; the spec stays at 0.9.x and nothing is re-valued.** Adds no `ODD-N.<PARAM>`, closes no TBD and touches no verdict. Two entries only. **(a) The 0.4954 yaw gain is superseded in shape, not in value** (§8.1 table): M-7 §5 measured the plant *while driving* and the achieved/commanded ratio **falls 0.482 → 0.436 → 0.341** with command magnitude, so no constant gain fits the channel and every 0.4954-derived margin is an optimistic bound at high demand; where the compression ends — and therefore the `R_min` the platform can actually reach — is **unmeasured**. `ODD-3.KAPPA_MAX` is unaffected: it is a property of the track, and whether the platform can reach it is a separate open hardware question. **(b) TBD-Q10 (`ODD-3.A_LAT_MAX`) is now closed *as open*:** Phase 5 ended without M-4, no lateral-accel measurement was ever taken, and none is planned — so the spec stays **below v1.0 by design**, exactly as D-33 predicted it would. The one open ODD parameter of this thesis is a hardware dependency, not an outstanding action item. |
...
- **Policy observation (track 'E', verdict of record).** The policy is a CNN over the **front camera** — the dedicated **Lane Cam** (IMX219-160 mirror, 640×360, HFOV ≈ 90° / 1.5708 rad, mounted at height `h ≈ 0.077 m`, pitch `0.30 rad` down — the *rendered* geometry, under which every verdict was scored; **the real module measures 77.89°**, M-6 17.08.2026, and deployment rectifies the hardware frame into this canonical model rather than re-parameterising it, docs/17 §8.3). Each native frame is grayscaled and resized to **84×84×1**, frame-stacked **×4** → an `84×84×4` tensor consumed by SB3 `CnnPolicy` (NatureCNN). The policy receives **no** ground truth, no LiDAR, and no authored centerline. `ODD-1.OBS_DIM` for track 'E' is therefore an **image**, not a scalar vector.
...
| Straight-line tracking | **≈0.99** of commanded, to 0.53 m/s | The forward axis transfers essentially 1:1. `ODD-1.V_MAX = 0.5 m/s` is 94 % of the platform's 0.53 m/s ceiling, so the actuator is a de-facto second limiter at the straight cap; `ODD-3.V_MAX_CURVE = 0.25` is comfortably reachable, i.e. **C-04 is physically exercisable on hardware** even though the 0.22 m/s trunk never fires it in sim. **But it was never exercised:** every physical run was capped at 0.22 m/s, so C-04 stayed inside its dead zone on commanded motion (D-75) while firing 58 and 40 cycles on **ZED velocity artefacts** at a reported 0.25–1.30 m/s (docs/17 §13.5). The rule's ODD-3 ceiling remains untested from above on hardware as well as in simulation. |
| **Yaw-rate tracking** | **0.4954 × commanded** (bench, in place) — **and compressive while moving: 0.482 → 0.436 → 0.341** | **The one genuine contradiction.** Gazebo's DiffDrive delivers ~1:1, so the simulated plant is **2× optimistic in rotation**. Every cage margin expressed through steering — C-01/C-02/C-03 corrections and C-06's slew bound — buys **half** the physical yaw it buys in sim, or less. Quantified in docs/14 §2.3a and docs/17 §2 item 4; carried as transfer risk **T2**. Compensated at deployment by `steering_to_yaw_rate_gain 0.8 → 1.615`, *not* by re-valuing any ODD parameter. **Superseded in shape by M-7 §5 (18.08.2026, docs/17 §6d):** measured on the circuit *while driving*, the ratio falls with command magnitude, so **no constant gain fits the channel** — one calibrated at moderate demand under-delivers exactly where C-01/C-02 correct. **Where that compression ends is unmeasured**, and with it the achievable `R_min`; scrub is also surface-dependent (0.150 in place on the circuit floor vs 0.4954 on the bench). Neither finding re-values an ODD parameter: `ODD-3.KAPPA_MAX` is a property of the *track*, and whether the platform can reach it is a separate, open, hardware question. |
