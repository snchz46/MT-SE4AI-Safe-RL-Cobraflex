...
\input{misc/thesis-commands.tex} % project-specific macros (IDs, verdicts, figures)
...
% Figures live in the shared manuscript figure pool, one level up. The chapters
% name them as the Markdown does, relative to that pool (auto/ included), so
% ../figures/ resolves them inside the repository and ./figures/ -- the copy
% that `python md2tex.py --zip` puts in the Overleaf project -- everywhere else.
\graphicspath{{../figures/}{./figures/}}

% Four parts, as in the DOCX build: a body file marked ::part:: in
% ../draft_v5_en becomes chapters/partN.tex (md2tex.py).
...
% The appendices are registers: tables of seven to nine columns that run over
% several pages. A page-breaking table cannot follow the margin column, which
% sits right of the text on odd pages and LEFT of it on even pages, so a
% longtable widened into the margin runs off the paper on every even page.
% The appendices therefore drop the margin column and set the text over the
% full width it occupied (107 + 8.2 + 49.4 = 164.6 mm). inner/outer are chosen
% so the block keeps the same outer edges as text + margin in the body, which
% keeps the running heads -- sized to that width by the class -- aligned.
\newgeometry{
	head=13.6pt,
	top=27.4mm,
	bottom=27.4mm,
...
% The Markdown reference list is the list of works cited; print all of it even
% where a work is named in the text without a year the converter can resolve.
\nocite{*}
