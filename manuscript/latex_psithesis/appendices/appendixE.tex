% Converted from: manuscript/draft_v5_en/back/E_appendix_cage.md
% Appendix E --- Cage parameters
%
% The YAML block below is the parameter file reproduced verbatim. It was
% extracted programmatically from the Markdown rather than retyped, because
% this appendix's whole value is that it is byte-for-byte what the running node
% consumes -- every run records this file's hash.
%
% It is set with `verbatim` rather than `lstlisting`: the comments contain
% Greek and arrows, and under LuaLaTeX + fontspec verbatim passes UTF-8 through
% to Iosevka (which has those glyphs), whereas listings tokenises bytes and is
% the less certain of the two. The cost is no syntax colouring.
...
\Cref{ch:08} is linked unambiguously to the exact configuration that produced it.
