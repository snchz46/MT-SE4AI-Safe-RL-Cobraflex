...
def fig_convergence(train_run: Path, out: Path) -> None:
    ...
    ax1.plot(ts, _smooth(rew), color="#1f77b4", lw=2, label="ep_rew_mean (suav. w=5)")
    ...
    ax2.plot(ts, _smooth(elen), color="#d62728", lw=2, label="ep_len_mean (suav.)")
    ax2.set_ylabel("ep_len_mean (pasos)", color="#d62728")
    ax2.tick_params(axis="y", labelcolor="#d62728")

    ax1.set_title("Fig. 7.1 — Convergencia PPO (run %s)" % train_run.name)
    fig.tight_layout()
...
def fig_trajectory(rl_run: Path, pd_run: Optional[Path], out: Path, laps: float = 2.0) -> None:
    ...
    ax.plot(pts[:, 0], pts[:, 1], "--", color="0.6", lw=1, label="centerline (carril)")
    ax.plot(rl_x[:n_rl], rl_y[:n_rl], color="#1f77b4", lw=1.6, label="PPO (RL)")
    ax.set_aspect("equal")
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
    ax.set_title(f"Fig. 7.2 — Trayectoria PPO sobre el óvalo (~{laps:g} vueltas)")
    ...
    ax2.text(0.02, 124, "medio-carril 122 mm", fontsize=7, color="0.4")
    ax2.set_xlabel(f"vueltas (warm-up {warm:g} recortado)")
    ax2.set_ylabel("|ey| (mm)")
    ax2.set_title("Fig. 7.2b — Error lateral RL vs PD")
