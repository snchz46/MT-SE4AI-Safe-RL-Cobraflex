...
*Figure 6.1 — The simulation environment of the state track: the reference oval with white lateral markings and a dashed centre separator, the vehicle on the start line, and the entity tree of the world, whose geometric parameters are the ones the operational domain declares.*
...
The reference system does not add nodes: it specialises the environment, as Figure 6.2 shows. The same training environment serves both tracks, and the camera branch is activated with a configuration switch. Four technical decisions deserve to be recorded.
