...
Todos los umbrales están en un fichero de parámetros versionado, no en el código. Esto tiene tres efectos prácticos. Primero, cada ejecución experimental guarda el *hash* del fichero junto con el resto de metadatos de reproducibilidad, así que cada resultado queda ligado sin duda a la configuración exacta que lo produjo. Segundo, los umbrales que todavía necesitan calibración física se marcan en el propio fichero, de modo que quien lo lea ve que son provisionales, en lugar de que esto quede perdido en algún punto de la documentación. Tercero, el versionado sigue una regla de compatibilidad hacia atrás: cuando se añade una funcionalidad nueva, sus valores por defecto deben dejarla inactiva para configuraciones anteriores. Así se puede volver a ejecutar una campaña antigua sin que una funcionalidad posterior cambie su resultado.
