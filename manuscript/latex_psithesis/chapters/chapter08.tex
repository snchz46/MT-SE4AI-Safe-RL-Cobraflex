...
The literal global verdict is \verdictnot, composed as \Cref{tab:8.1} shows, and it is blocked by two class A requirements --- heading stability and predictive time to lane departure --- exclusively through a single scenario and a single clause. It is worth breaking it down, because the difference between \enquote{the system is not safe} and what really happens is the whole difference of the chapter:
...
\begin{table*}[t]
	...
	\begin{tabularx}{\widefigurewidth}{@{}L{42mm}cY@{}}
		...
		Class B with a \textbf{literal} failure & 1 / 4 & \sr{011} --- the same inherited clause; satisfied on its own metric (\qdeg{3.77} $<$ \qdeg{5})\\
		Class B \textbf{not satisfied} & 1 / 4 & \sr{010} --- co-activation grid; the one negative verdict of the work (\Cref{sec:8.6})\\
		Class B closed out of band & 2 / 4 & \sr{006} and \sr{009} --- satisfied on their own metrology (\Cref{sec:8.6.1})\\
		\bottomrule
	\end{tabularx}
\end{table*}

In the blocking scenario, the only clause violated over its thirty \enf{} runs is the one on heading recovery time (\Cref{tab:8.2}). The safety clauses are not touched in any of them:
...
\begin{table}[t]
	...
	\begin{tabularx}{\textwidth}{@{}Yrr@{}}
		...
		\midrule
		Emergency stops & 0 & ---\\
		Maximum lateral excursion & \qm{0.043} & \qm{0.16}\\
		Maximum heading error & \qdeg{14.2} & \qdeg{25}\\
		Maximum heading standard deviation & \qdeg{3.77} & \qdeg{5}\\
		\bottomrule
	\end{tabularx}
\end{table}

The two requirements are therefore satisfied on their own documented criterion: the heading one requires that the error does not exceed \qdeg{25}, and the measured maximum is \qdeg{14.2}; the predictive one requires a time margin that is never compromised, with the vehicle at a quarter of the lateral limit. The \qs{2.0} recovery clause is an inherited performance requirement from an earlier library, not the safety predicate of either of them.
...
\begin{table}[t]
	...
	\begin{tabularx}{\textwidth}{@{}Yrr@{}}
		...
		\midrule
		\tabhead{Enforcement} (cage active) & 0 & 56\\
...
In the clean nominal scenario the cage is latent: the policy drives 5.32 laps with \qmm{8.6} mean lateral error, zero emergencies and zero safety interventions; only the rate limiter acts. But the contrast between modes, scenario by scenario (\Cref{tab:8.4}), shows where it stops being latent:
...
\textbf{The only thing that differs between modes is the rate limiter} (\Cref{tab:8.5}). Same policy, same layout, same speed at the apex:
...
\begin{table}[t]
	...
	\begin{tabularx}{\textwidth}{@{}L{40mm}rY@{}}
		...
		Applied variation per cycle & $\le$ 0.15 & up to 2.0\\
		Maximum lateral error & \qmm{36} & \qmm{145} $\rightarrow$ off the road\\
		\bottomrule
	\end{tabularx}
\end{table}
...
The one on actuation smoothness is verified directly over the trace of the committed command: in \enf{}, 840 out of 840 runs respect the per-cycle limit; in \mon{} only 263 out of 945. It is, incidentally, the most direct measure of the value of the limiter.

The one on \emph{liveness} is closed on its own metrology. Its scenario --- a two-arm meta-test that injects an adverse incentive to stop --- was excluded from the campaign because it is independent of the policy and had already been closed separately, with three measured parts: the nominal policy never stops; a deliberate attempt to force it to stop does not succeed, which is positive evidence of the training mitigation; and the detector does trigger on a real stop injected by script. A mitigation that works, a pathology that is resisted, and a healthy metric.
...
\begin{itemize}
	\item \textbf{A single seed in the verdict campaign.} The reference campaign uses one seed; the variability between seeds is characterised separately (\Cref{sec:7.4}) and shows that the behaviour is not homogeneous. The generalisation of the verdict to other seeds is not established.
