...
This section justifies each choice against the discarded alternatives, leaving an auditable record of decisions that would otherwise remain implicit. The complete detail — including the alternatives evaluated for middleware, RL library, physical platform and instrumentation — is given in **Appendix C**.
...
The framework does not replace the standards: it articulates them. Each adaptation has an identifiable normative anchor, summarised below; the complete mapping, clause by clause, is given in **Appendix C**.
