...
%
% NOTE on numbering: the source Markdown numbers its headings by hand and the
% numbering is inconsistent -- the first division runs C.1, C.2, C.4, C.5, C.6,
% C.7 (C.3 is skipped, and the subsection headed "C.2 Middleware" is about the
% RL algorithm, not middleware), and the second division restarts at C.1. The
% manual numbers are dropped here and LaTeX numbers the sections itself, so the
% appendix comes out consistently numbered. No heading text was changed, and
% nothing in the thesis cross-references these numbers.
...
Alternatives considered and discarded. \textbf{CARLA} \parencite{dosovitskiy2017} is the strongest candidate and the default choice in
...
operational obstacle for an individual thesis. \textbf{Highway-Env} and other environments
derived from Gym, without realistic sensors and with an abstract observation
space, are not suitable for camera-based policies. \textbf{LGSVL}, a
...
Alternatives considered and discarded: \textbf{SAC} \parencite{haarnoja2018}
...
adds complexity to the experiment design; \textbf{DDPG / TD3} (deterministic
\emph{off-policy}) are less stable than SAC and have been superseded by it in
almost all benchmarks; \textbf{A3C / A2C} are less sample-efficient
