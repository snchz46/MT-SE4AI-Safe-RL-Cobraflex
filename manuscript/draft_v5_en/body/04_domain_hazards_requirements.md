...
The specification of the operational domain is stratified into the four nested domains of Figure 4.1, each of which isolates one axis of complexity. This makes it possible to attribute an observed change in safety or performance to a single cause and not to a confounded combination:
...
The register, summarised in Table 4.1, consolidates twelve hazards: nine at the system level, common to both tracks, and three specific to the camera track. The numbering is stable: an assigned identifier is neither reused nor renamed, even if the hazard is discarded in later reviews. The table presents the compact form; the extended register, with the root cause hypothesis and the operational consequence of each entry, is in Appendix A.
...
The register contains fourteen requirements. Table 4.2 presents them in compact form; the complete *rationale* for each one — including the derivation of each threshold and the discussion of the values marked as provisional while waiting for physical calibration — is in Appendix B.
...
One threshold in the table deserves a note, because it is the only one the implementation reparameterises. SR-007 fixes the maximum age of the state observation at `200 ms`, which is four cycles of the control loop at 20 Hz. The corresponding C-05 trigger runs at 10 Hz on the camera track and uses `0.5 s`: the same tolerance expressed in cycles — five — and not a relaxation of the requirement. The equivalence is recorded in the parameter file and in Appendix D; it is flagged here so that the reader does not read a discrepancy where there is a declared reparameterisation.
